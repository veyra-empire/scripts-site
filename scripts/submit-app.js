(function() {
  ...
  // ─── DOM refs ─────────────────────────────────────────────────────────────
  var session = null;
  // Holds every app/extension in the manifest. Submission flow is open:
  // any tiered member can update or delete any entry, lmv reviews each PR.
  // Each entry includes `author` and `ownerDiscordId` for the cross-author
  // dropdown labels and author prefill (parallel to submit.js / submit-resource.js).
  ...
  function prefillFromSelectedUpdate() {
    ...
    var r = availableExtensions.find(function(x) { return x.id === selectedId; });
    if (!r) return;
    ...
    if (!fieldUrl.value)         fieldUrl.value         = r.driveUrl    || '';
  ...
  // apps may legitimately be unversioned.
  function updateVersionWarning() {
    if (!versionWarning) return;
    if (currentMode() !== 'update') { versionWarning.hidden = true; return; }
    var selectedId = idUpdate.value;
    var r = selectedId && availableExtensions.find(function(x) { return x.id === selectedId; });
  ...
  var ERROR_MESSAGES = {
    ...
    'duplicate-id':       "An app with that ID already exists. Pick a different ID, or switch to Update mode if you meant to update the existing entry.",
    'not-found':          "No app with that ID exists. Switch to New mode, or pick a different app to update.",
    'not-owner':          "You don't own this app. Only the original submitter (or lmv) can update it.",
    'invalid-id':         'App ID must be lowercase letters, digits, and hyphens only (2-49 chars).',
    'invalid-min-tier':   'Min tier must be Probationary, Full Member, or Tester.',
    'invalid-url':        'Drive folder URL must start with https://',
    'missing-url':        'Please provide the Drive folder URL.',
    'missing-name':       'Please provide a display name for this app.',
    'missing-reason':     'Please explain why this app should be deleted (at least 10 characters).',
  ...
  function handleSubmit(e) {
    ...
    if (!id) { showError('Pick an app ID.'); return; }
    ...
    if (mode === 'new' && !url) { showError('Please provide the Drive folder URL.'); return; }
    if (url && !/^https:\/\//i.test(url)) { showError('Drive folder URL must start with https://'); return; }
  ...
  function init() {
    ...
    // Fetch every app/extension in the archive (submission flow is
    // open; lmv reviews cross-author submissions via the PR body banner).
    // Also fetches the caller's submission-rate-limit status for the
    // cancel banner.
    jsonp(PROXY_URL + '?api=my-extensions&session=' + encodeURIComponent(session.sid))
      .then(function(res) {
        if (res && res.error === 'expired') { show('state-unauthenticated'); return; }
        availableExtensions = (res && res.extensions) || [];
        if (!availableExtensions.length) {
          // Degenerate case: empty extensions section. Update / Delete
          // have nothing to pick. Falls through to the New mode workflow.
          ...
          [idUpdate, idDelete].forEach(function(sel) {
            ...
            availableExtensions.forEach(function(r) {
      ...
      .catch(function() {
        // Still allow submission in new mode even if my-extensions fails.
        modeUpdateLabel.classList.add('disabled');
        modeUpdateLabel.querySelector('input').disabled = true;
        modeDeleteLabel.classList.add('disabled');
        modeDeleteLabel.querySelector('input').disabled = true;
